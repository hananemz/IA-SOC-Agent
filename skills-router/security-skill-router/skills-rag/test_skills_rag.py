...
class LocalRagTests(unittest.TestCase):
    def test_tokenization_is_case_insensitive(self):
        self.assertEqual(skills_rag.tokens("Elastic ES|QL"), ["elastic", "es", "ql"])

    def test_only_operational_corpus_is_indexed(self):
        index = skills_rag.build_index()
    ...
    def test_search_returns_relevant_router_evidence(self):
        results = skills_rag.search("Investigate suspicious PowerShell execution", top_k=5, decision={"platform": "elastic", "skill": "security-alert-triage"})
    ...
    def test_platform_and_skill_isolation(self):
        splunk = skills_rag.search("authentication brute force", decision={"platform": "splunk", "skill": "splunk-authentication"})
        self.assertTrue(all(item["platform"] in {"splunk", "any"} for item in splunk["results"]))
        elastic = skills_rag.search("authentication brute force", decision={"platform": "elastic", "skill": "elasticsearch-authn"})
    ...
    def test_adaptive_result_budget_is_configurable(self):
        result = skills_rag.search("authentication failed login", top_k=8, decision={"platform": "elastic", "skill": "elasticsearch-authn"})
    ...
    def test_no_decision_means_no_retrieval(self):
        self.assertEqual(skills_rag.search("Investigate suspicious PowerShell") ["results"], [])

    def test_skill_context_is_deduplicated(self):
        initial = skills_rag.search("PowerShell telemetry gap troubleshooting", decision={"platform": "elastic", "skill": "security-alert-triage"})
        result = skills_rag.search("PowerShell telemetry gap troubleshooting", decision={"platform": "elastic", "skill": "security-alert-triage"}, skill_context=initial["results"][0]["snippet"])
    ...
    def test_empty_result_does_not_invent_context(self):
        result = skills_rag.search("florp zibble", decision={"platform": "elastic", "skill": "security-alert-triage"})
    ...
    def test_result_contract_has_no_authority_or_evidence_fields(self):
        result = skills_rag.search("PowerShell investigation", decision={"platform": "elastic", "skill": "security-alert-triage"})
