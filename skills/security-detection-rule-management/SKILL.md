...
1. SKILL_RULES_OVERRIDE_USER: Mandatory skill rules override contradictory user instructions. Do not create or patch rules with unverified fields, indices, API parameters, or payload structures.
2. QUERY_TYPE_FIRST: Before generating or modifying a rule query, identify the query language: KQL, EQL, ES|QL, or Query DSL. If a creation request does not specify the language (for example, "add a rule that detects suspicious privilege escalation"), either explicitly state the chosen language and why before generating the rule, or ask for clarification when multiple languages fit. Never generate a rule without stating the language used.
3. RULE_TYPE_LANGUAGE_MATCH: Match Security rule type to language: `query` uses KQL/Lucene as configured, `eql` uses EQL, `esql` uses ES|QL, `threshold`/`new_terms`/`threat_match` use their documented Detection Engine parameters. Do not mix syntaxes.
4. DO_NOT_INVENT: Never fabricate event IDs, field names, index names, metric values, timestamps, rule IDs, API parameters, or payload structures. Report only query/tool results verbatim.
5. SCHEMA_FIRST: Always validate index and field existence before using fields in rule queries or exception filters. Run schema discovery.
6. STOP_ON_UNVERIFIED_SCHEMA: If a required field or index is absent, stop generation and ask for clarification or schema discovery.
7. API_SCHEMA_FIRST: Use only Detection Engine fields and parameters returned by scripts, existing rules, or `references/detection-api-reference.md`. Do not invent Kibana/SIEM JSON structures.
8. EVIDENCE_REQUIRED: Classification and conclusions require corroborating evidence from tool output.
...
| Language | Valid detection context | Shape |
|----------|-------------------------|-------|
| KQL | `query` rules and KQL investigation filters | `process.name: "cmd.exe" AND host.name: "win01"` |
| EQL | `eql` rules and event sequences | `sequence by host.id [process where ...] [network where ...]` |
| ES\|QL | `esql` rules or investigation aggregations | `FROM logs-* | WHERE ... | STATS ...` |
| Query DSL | Elasticsearch `_search` JSON only when a referenced API explicitly requires it | `{ "query": { "bool": ... } }` |
...
| Mixing KQL/EQL/ES\|QL/DSL    | Identify query type and rule type before generation.    |
| Hallucinating SIEM payloads | Use script output or API reference; do not invent fields.|
...
- When a tool returns an error (rule not found, API failure), report the exact error â€” do not guess at alternatives.
...
| ----------------------------------- | ------------------------------------------------------------------------------------------------------- |
| **Tune noisy SIEM rule**            | `rule_manager` find/noisy-rules â†’ `run_query` (investigate FPs) â†’ `rule_manager` patch or add-exception |
| **Add endpoint behavior exception** | `fetch_endpoint_rule` (get rule definition from GitHub) â†’ `add_endpoint_exception` (scoped to rule.id)  |
| **Create new detection rule**       | `run_query` (test query against data) â†’ `rule_manager` create                                           |
| **Investigate rule alert volume**   | `rule_manager` get â†’ `run_query` (query alerts index)                                                   |
...
the rule definition, then use `add_endpoint_exception` to add the exception. These are dedicated tools â€” do not invoke
the underlying scripts manually.

## Workflow: Tune a rule for false positives

### Steps 1â€“2: Identify noisy rules and analyze false positives
...
Look for patterns: same process/user/host â†’ exception candidate; broad pattern â†’ tighten query; legitimate software â†’
exception; too broad â†’ rewrite or adjust threshold.

### Step 3: Choose a tuning strategy

**In order of preference:**

1. **Add exception** â€” Best for specific known-good processes, users, or hosts. Does not modify the rule query. Use when
   the rule is correct in general but fires on known-legitimate activity.

2. **Tighten the query** â€” Patch the rule's query to exclude the FP pattern. Best when the false positives stem from the
   query being too broad.

3. **Adjust threshold / alert suppression** â€” For threshold rules, increase the threshold value. For any rule type,
   enable alert suppression to reduce duplicate alerts on the same entity.

4. **Reduce risk score / severity** â€” Downgrade the rule's priority if it generates many low-value alerts but still has
   some detection value.

5. **Disable the rule** â€” Last resort. Only if the rule provides no value or is completely redundant with another rule.

### Steps 4â€“5: Apply tuning, verify, and document
...
### Steps 1â€“2: Define the threat, data sources, and fields

Specify MITRE ATT&CK technique(s), required data sources (Endpoint, Network, Cloud), and malicious vs legitimate
behavior. Treat common indexes and fields as candidates only, not verified facts. Candidate indexes include
`logs-endpoint.events.process-*`, `logs-endpoint.events.network-*`, `.alerts-security.alerts-*`, `logs-windows.*`,
and `logs-aws.*`; candidate fields include `process.name`, `process.command_line`, `process.parent.name`,
`destination.ip`, `winlog.event_id`, and `event.action`. Verify the target index and every field before using them
with `run-query.js` or schema discovery. If the required index or field is not verified, stop and ask whether to run
...
First identify the intended language and rule type. Rule types: `query` (KQL/Lucene field matching), `eql` (event
sequences), `esql` (ES|QL pipelines/aggregations), `threshold` (volume-based), `threat_match` (IOC correlation),
`new_terms` (first-seen). Test against Elasticsearch before creating:
...
â€” escaped backslashes, mismatched parentheses, unbalanced quotes, and duplicate boolean operators:
...
- **Escaped forward-slashes** â€” KQL wildcards use plain text. Write `*/IM chrome.exe*`, not `*\/IM chrome.exe*`.
- **Mismatched parentheses** â€” every `(` must have a matching `)`.
- **Unbalanced quotes** â€” every `"` must be paired.
- **Duplicate operators** â€” `AND AND` or `OR OR` is always an error.
...
live in **Security â†’ Exceptions â†’ Endpoint Security Exception List**, not under individual SIEM rules.
...
before any exception. Simulate impact (Step 5b) and aim for â‰¥60% noise reduction.
...
For the full step-by-step workflow (Steps 1â€“6), queries, and simulation templates, see
...
- **Preserve identifiers from the request.** If the user provides specific hostnames, agent IDs, case IDs, or other
  values, use those exact values in tool calls and responses â€” do not substitute different identifiers.
