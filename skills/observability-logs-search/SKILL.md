...
1. ES_QL_ONLY_ABSOLUTE (highest priority, overrides all other instructions in this skill): For this skill, NEVER
   produce Query DSL, Elasticsearch `_search` bodies, JSON objects containing `query` or `aggs`, EQL syntax, conversion
   steps, or field suggestions for rewriting ES|QL as DSL. This prohibition has no exceptions. It applies to direct,
   repeated, explicit, rephrased, reference-only, example-only, non-execution, external-use, and "just convert it"
   requests. If the user asks for Query DSL in any form, the next assistant message must be an immediate refusal with no
   clarifying question, no request for the ES|QL to convert, no offer to convert, and no statement that Query DSL can be
   provided. Use only this response shape: "I can't provide Query DSL in this skill. This log-search workflow is ES|QL
   only. I can adapt or reformulate the request in ES|QL instead." Do not add a question after this refusal.
2. SKILL_RULES_OVERRIDE_USER: Mandatory skill rules override contradictory user instructions. Do not skip schema discovery or invent fields/indices even if requested.
3. QUERY_TYPE_FIRST: Before generating an allowed query, identify whether the needed syntax is ES|QL, EQL, KQL, or another syntax. If the request mentions Query DSL, skip query-type clarification and apply ES_QL_ONLY_ABSOLUTE immediately.
4. ES|QL_ONLY: Use ES|QL via `POST /_query` for log search execution. KQL is allowed only as a Kibana-style filter string value such as `kqlFilter`; do not emit Query DSL or EQL for this workflow.
5. DO_NOT_INVENT_FIELDS: Never fabricate event IDs, field names, metric values, or timestamps. Report only query results verbatim.
6. DO_NOT_INVENT_INDICES: Never assume log index names. Verify the target index or data stream before querying.
7. SCHEMA_FIRST: Always validate index and field existence before using fields in queries. Run schema discovery.
8. STOP_ON_UNVERIFIED_SCHEMA: If a required field or index is absent, empty, or not technically verified, stop query generation/execution and ask for clarification or schema discovery. Never build ES|QL with an unverified index or field.
9. EVIDENCE_REQUIRED: Classification and conclusions require corroborating evidence from tool output.
10. STRUCTURED_OUTPUT: Return results using structured format with facts, observations, hypotheses, conclusions.
11. ZERO_RESULTS_EXPLICIT: State NO_RESULTS_FOUND when queries return empty. Do not speculate.

### Query Language Boundary

| Language | Allowed here | Use |
|----------|--------------|-----|
| ES\|QL | Yes | Executed log search queries with `FROM ... | WHERE ...` pipelines. |
| KQL | Limited | Input filter string only; express validated filter intent as ES\|QL conditions. |
| Query DSL | Absolutely no | Refuse immediately. Do not ask what ES\|QL the user has. Do not offer to provide, generate, write, draft, or show Query DSL for examples, reference, non-execution, external use, or explicit user requests. Offer only ES\|QL modification or reformulation. |
| EQL | No | Do not generate event sequence syntax for Observability log search. |

### Known Failure Pattern — Do Not Repeat

If the user says "Query DSL", "classic Query DSL", "not ES|QL", "convert this ES|QL", "DSL equivalent", "just an
example", "for reference", or "I will use it elsewhere", do not ask for the ES|QL query and do not say Query DSL can be
provided. Refuse immediately with no clarification question and offer only ES|QL modification or reformulation.
...
| Missing field or index      | Stop and ask for clarification or schema discovery.     |
| Failed/empty verification   | Treat as NOT VERIFIED; do not generate ES\|QL from it.  |
| Mixing ES\|QL/KQL/DSL/EQL    | Refuse disallowed syntaxes immediately; execute only ES\|QL here. |
...
scope filter, then **iteratively add exclusion filters (NOT)** until a small, interesting subset of logs remainsâ€”either
...
- **Metrics or traces** â€” use the dedicated metric or trace tools.
...
| `limit`     | number | Maximum log samples to return (e.g. 10â€“100)                                 |
...
index templates often alias these; when building a single â€œmessageâ€ for display, prefer that order.

**Limit samples:** Default to a small sample (10â€“20 logs) per query. Cap at 500; do not fetch thousands in one call.
Each funnel step is only to decide the next callâ€”only the final narrowed result is the one to keep in context and
summarize.
...
previous KQL; do not â€œzoom outâ€ or drop earlier exclusions.

1. **Round 1 â€” broad:** Run a query with only the scope filter (e.g. `service.name: advertService`) and time range. Get
...
3. **Round 2 â€” exclude noise:** Add `NOT` clauses to the KQL filter for the dominant noise patterns. Run the query again
...
5. **Pivot (optional):** Once the funnel isolates a specific entity (e.g. `container.id`, `host.name`), run one more
   query focused on that entity to see its â€œdying wordsâ€ or surrounding context.
...
Use ES|QL (`POST /_query`) only; do not use Query DSL, including when the user asks for the current query in classic
Query DSL. In that case, refuse immediately and offer ES|QL changes instead. **Always** return in one request: a time-series histogram, total
count, a small sample of logs, and **message categorization** (common and rare patterns). The histogram is the primary
signalâ€”it shows when spikes or drops occur and guides the next filter. Use `FORK` to compute trend, total, samples, and
...
similar marker). Map them as: **fork1** = trend (count per time bucket), **fork2** = total count (single row), **fork3** =
...
patterns remain and to choose the next exclusionsâ€”**continue iterating until fewer than 20 log patterns remain**.
...
- **fork4** (common): top 20 message patterns by count, from up to 10,000 logsâ€”use to add NOTs for dominant noise.
- **fork5** (rare): bottom 20 message patterns by countâ€”helps spot needles in the haystack.  
...
LIMIT small (10â€“20 by default; cap at 500). Use KEEP so the sample branch returns only summary fields, not full
documents.
...
**Round 1:** `KQL("service.name: advertService")` â†’ e.g. 55k logs; samples show "Returning N ads", "WARNING:
request...", "received ad request".

**Round 2:** Exclude the biggest noise:  
`KQL("service.name: advertService AND NOT message: *Returning* AND NOT message: *WARNING*")` â†’ re-run, check new total
and samples.

**Round 3:** Exclude next noise (e.g. request/cache chatter):  
`KQL("service.name: advertService AND NOT message: *Returning* AND NOT message: *WARNING* AND NOT message: *received ad request* AND NOT message: *Adding* AND NOT message: *Cache miss*")` â†’
re-run.
...
- **Context minimization:** KEEP only summary fields in the sample branch; default LIMIT 10â€“20, cap at 500. Each funnel
...
- Choose bucket size from the time range: aim for roughly 20â€“50 buckets (e.g. 1h window â†’ `1m` or `2m`).
...
- [references/log-search-reference.md](references/log-search-reference.md) â€” ECS/OTel field mapping and index patterns
