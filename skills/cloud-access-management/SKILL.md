...
> **Prerequisite:** This skill assumes the **cloud-setup** skill has already run â€” `EC_API_KEY` is set in the
...
### Step 1 â€” Identify the components
...
### Step 2 â€” Check if a predefined role fits

Consult the [predefined roles table](#predefined-roles) below. Prefer predefined roles â€” only create a custom role when
predefined roles do not provide the required granularity.

### Step 3 â€” Check existing state
...
> the project â€” the Viewer stack role is broader than most custom roles and will override the restrictions you intended.
...
- **Custom roles** are created in the project via the Elasticsearch security API (`create-custom-role`) and assigned via
  ...
  **only** the specified custom role on SSO â€” not the default stack role for their Cloud role.
...
- Cloud API keys can also use `application_roles` to gain ES/Kibana API access on Serverless projects. See
  [Cloud API Keys â€” ES and Kibana API Access](#cloud-api-keys--es-and-kibana-api-access) below for details.
...
2. Invite the user to the organization if they are not already a member (`invite-user`). Do **not** include project role
   assignments in the invitation â€” the custom role assignment in the next step handles project access.
...
## Cloud API Keys â€” ES and Kibana API Access

Cloud API keys can now optionally access Elasticsearch and Kibana APIs on Serverless projects, in addition to the Cloud
API. This enables a single credential for both control plane (Cloud API) and data plane (ES/Kibana API) operations â€” for
...
Custom roles must be created individually in each project where the key should have access â€” if a referenced custom role
...
by design for backward compatibility â€” existing keys without `application_roles` continue to work as Cloud-only keys.

### Scoping modes

- **Project-scoped** (preferred) â€” grants access to specific projects or all projects of a given type. Uses the
...
- **Organization-scoped** â€” grants access to **all current and future projects** in the organization. Uses the
...
> have that custom role defined (via `PUT /_security/role/{name}`), the key silently gets no access to that project â€” no
...
> intent before proceeding â€” org-scoped access applies to future projects too. If the user specifies a custom role name
...
**Organization-scoped key with admin ES access** (access to ALL projects â€” use with caution):
...
> `application_roles`. Unlike users, API keys must have explicit `application_roles` to access the stack â€” the `role_id`
...
For custom role access, use `assign-custom-role` after the user has accepted the invitation â€” do not combine a
...
instead of the raw secret. Tell the user to retrieve the key from that file â€” it is shown only once. When the CI
...
# 2) Invite user to the organization (no project roles â€” custom role handles access)
python3 skills/cloud/access-management/scripts/cloud_access.py invite-user \
...
permissions when they SSO into the project. Do not also assign `viewer` as a separate Cloud role for this project â€”
...
You are an Elastic Cloud access management specialist. Follow these rules:

1. **DO_NOT_INVENT**: Never fabricate org IDs, user IDs, project IDs, or API responses. Report only tool output verbatim.
2. **VERIFY_FIRST**: Always run list-members before any modify operation.
3. **ZERO_RESULTS_EXPLICIT**: Always state NO_RESULTS_FOUND when APIs return empty arrays.
4. **SECRET_HANDLING**: Never print, echo, or log API keys. Use _secret_file path for unredacted values.
5. **DETERMINISTIC_OUTPUT**: Apply the role selection decision tree exactly.

### Role Selection Decision Tree (Priority Order)

| Priority | User Access Requirement           | Role to Assign     |
|----------|-----------------------------------|--------------------|
| 1        | Full project admin                | admin (predefined) |
| 2        | Read-only console + ES access     | observer           |
| 3        | Read-only console only            | viewer             |
| 4        | Custom index/Kibana privileges    | custom role        |
| 5        | No project access, org-level only | no project role    |

### Error Response Matrix

| HTTP | Cause                        | Action                                          |
|------|------------------------------|--------------------------------------------------|
| 401  | Expired/invalid EC_API_KEY   | Stop. Instruct user to re-run cloud-setup.       |
| 403  | Insufficient org permissions | Stop. Report required permission.                |
| 404  | User/project not found       | Stop. Run list-members/list-projects to verify.  |
| 422  | Invalid role schema          | Log schema. Do NOT retry with modified payload.  |

### Hallucination Guards

| Risk                        | Guard Rail                                           |
|-----------------------------|------------------------------------------------------|
| Inventing org/user IDs      | Query list-members first; use returned IDs only.     |
| Over-scoping API keys       | Apply principle of least privilege per decision tree.|
| Missing expiration          | Always set --expiration matching task lifetime.      |


## Guidelines

- If `EC_API_KEY` is not set, do not prompt the user â€” instruct the agent to invoke **cloud-setup** first.
...
- Cloud API keys inherit roles at creation and cannot be updated â€” revoke and recreate to change roles.
- **API key hygiene â€” minimize, scope, and expire:**
  - Before creating a key, always run `list-api-keys` and check whether an existing key **for the same purpose or task**
    ...
    purposes (for example, two separate CI pipelines) are legitimate â€” the goal is to avoid redundant keys for the same
    task.
...
- **Custom role security â€” do not over-assign:** Never assign a predefined Cloud role (for example, `viewer`) for a
...
- If a custom role exists but the user cannot access the project, verify the role was assigned with `assign-custom-role`
  (which uses `application_roles` in the Cloud API). Creating a custom role alone does not grant project access â€” the
