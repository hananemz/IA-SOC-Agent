...
1. DO_NOT_INVENT: Never fabricate IDs, names, endpoints, APIs, realm options, privilege names, settings, security configuration, or API responses. Unknown values must be verified or clearly marked as assumptions.
...
4. VERIFY_USER_VALUES: Treat user-provided privilege names, realm names, role names, API names, index names, data streams, and IDs as untrusted until verified. If verification fails, state that the value remains unverified.
...
- **Always use environment variables.** All code examples in this skill reference environment variables (e.g.
  ...
  in a `.env` file in the project root â€” never prompt for the value directly.
...
| **Credentials**       | Depends on the realm â€” see the methods below                                                                                |
...
not be visible to agents running in a separate shell session â€” the `.env` file is the reliable default. **Never ask the
user to paste credentials into the chat** â€” secrets must not appear in conversation history.
...
Authenticates against an external LDAP directory using username and password. Self-managed only â€” not available on ECH
...
The request is identical to native â€” Elasticsearch routes it to the LDAP realm via the realm chain.

#### Active Directory

Authenticates against an Active Directory domain. Self-managed only â€” not available on ECH or Serverless. Similar to
...
HTTP layer. On ECH, PKI support is limited â€” check deployment settings. Not available on Serverless. Best for
...
organization level and not configurable per project. Not usable by standard REST clients â€” the browser-based redirect
...
- **`id_token`** (default) â€” OpenID Connect ID tokens for user-on-behalf-of flows.
- **`access_token`** â€” OAuth2 client credentials for application identity flows.
...
Authenticates using Kerberos tickets via the SPNEGO mechanism. Self-managed only â€” not available on ECH or Serverless.
...
The `--negotiate` flag enables SPNEGO. The `-u :` is required by curl but the username is ignored â€” the principal from
...
The response contains `id`, `api_key`, and `encoded`. Store `encoded` securely â€” it cannot be retrieved again.
...
Prefer API keys for automated workflows â€” they support fine-grained scoping and independent expiration. For Kibana SSO,
...
- Native users do not exist â€” users are managed at the Elastic Cloud organization level.
