...
description: >
  ...
  â€” the legacy ECS Kubernetes integration shape is out of scope.
metadata:
  author: elastic
...
- The legacy Elastic Agent Kubernetes integration (`metrics-kubernetes.*`, `logs-kubernetes.*`, `kubernetes.*` fields).
  Being deprecated â€” do not author queries against these paths.
- APM-layer analysis (service SLO breaches, transaction error rates, upstream dependency health). Different domain â€”
...
likely not collected or the pod has no recent lines â€” this does _not_ mean "dependency unavailable" or any other
specific failure mode. Report `no_logs_available` and weight remaining signals accordingly.

**Empty dependency data â‰  upstream healthy.** Services without APM instrumentation (load generators, workers) emit no
...
attribute causation when (a) one service's degradation clearly precedes the other's, and (b) the delta is large (>5Ã—
error rate, >3Ã— latency).

**OOMKilled â‰  memory leak by default.** The limit may simply be undersized for the workload's working set. Compare
against a 7-day baseline at the same hour-of-day before claiming a leak.

**Error-termination â‰  application bug by default.** Check `k8s.pod.cpu_limit_utilization` first. CFS throttling driving
liveness probe timeouts is the most common misdiagnosis in this space.

**Average CPU hides throttling.** A pod can look healthy at 40â€“60% average `cpu_limit_utilization` while being throttled
...
| `k8s.pod.memory_limit_utilization`               | kubeletstatsreceiver        | 0.0â€“1.0+ (can exceed 1 transiently before OOM)          |
| `k8s.pod.cpu_limit_utilization`                  | kubeletstatsreceiver        | 0.0â€“N (frequently >1 under CFS throttling)              |
...
| `k8s.pod.status_reason`                                      | Same â€” optional metric on k8sclusterreceiver.                                                              | Infer from events: `Evicted`, `NodeLost`, `Preempted`.                                                                        |
...
`AVG(field)` gives sum/value_count. Do not use `SUM(transaction.duration.summary)` as an event-count proxy â€” it returns
total duration.
...
| ----------------------------------- | -------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **OOMKilled**                       | `last_terminated_reason == "OOMKilled"` + `memory_limit_utilization â†’ 1.0`       | Monotonic rise (leak) vs. load-driven spike? Compare current trend to 7-day baseline. Check heap metrics (JVM, Go, Node) for GC pressure.                                                                                                                 |
| **CPU throttling â†’ Error exit**     | `cpu_limit_utilization > 1.0` + `last_terminated_reason == "Error"`              | Liveness/readiness probe timeouts from CFS throttling. Average CPU can look fine (40â€“60%) while p99 throttle is severe. Check probe timeouts vs observed startup/health latency.                                                                          |
...
| **CrashLoopBackOff (generic)**      | `BackOff` events + rising `k8s.container.restarts`                               | Branch on `last_terminated_reason` â€” this is a meta-mode. OOMKilled â†’ memory path; Error â†’ logs + throttling; ContainerCannotRun â†’ image/exec.                                                                                                            |
...
| **Termination signal race**         | Brief 5xx bursts correlated with rolling deploys                                 | Endpoint removal races termination. New requests can hit the pod after SIGTERM starts. NGINX gotcha: `STOPSIGNAL SIGTERM` triggers _fast_ shutdown, not graceful â€” use `STOPSIGNAL SIGQUIT` for graceful drain. Check ingress 502 rate vs rollout timing. |
...
| **Resource eviction**               | `status_reason == "Evicted"` + `condition_memory_pressure == 1` on node | Node-level noisy neighbor. QoS order: BestEffort â†’ Burstable â†’ Guaranteed. Identify which pod drove node memory up.                |
| **Node affinity/selector conflict** | Mass unschedulable pods after label change                              | K8s events show `FailedScheduling`. Often triggered by cluster upgrades (e.g. `node-role.kubernetes.io/master` â†’ `control-plane`). |
...
| **CoreDNS OOMKill**         | CoreDNS restarts + cluster-wide DNS timeouts in app logs | Default CoreDNS memory (~170Mi) insufficient under query amplification (ndots:5, each external lookup â†’ ~10 lookups). |

### When classification is ambiguous

Real incidents often match two modes. Examples:

- OOMKilled pod with simultaneous CPU throttling â€” memory usually drives the kill, but verify by checking whether memory
  or CPU hit limit first.
- Stuck rollout with HPA dampening and resource quota near-exhaustion â€” both can freeze a deploy. Check which constraint
  is binding.
- Node NotReady with pods that were already crashing â€” the node issue may be incidental.
...
- **Monotonic rise over 30â€“60 min** â†’ leak. Check GC metrics for the language: JVM `jvm.gc.duration`, Go
...
- **Diurnal / load-correlated spikes** â†’ load-driven, not leak. Consider HPA tuning or limit increase.
- **Hits 1.0, then restart** â†’ OOMKilled confirmed. Exit code 137 (SIGKILL) in app logs consistent.

### CPU

- `cpu_limit_utilization > 1.0` sustained â†’ CFS throttling. Node has spare CPU; the pod is quota-blocked.
- Symptoms of throttling (not the throttle metric itself): liveness probe timeouts, p99 latency 4â€“16Ã— p50, queue
...
- `restarts > 0` recently â†’ workload has been restarting. Don't read magnitude into the count (see _Restart count is
  boolean_); confirm the pattern from K8s `Killing` / `BackOff` event timestamps in `logs-k8seventsreceiver.otel-*`.
- Restarts correlated with memory pressure (`memory_limit_utilization â†’ 1.0`) â†’ OOMKilled path.
- Restarts without memory/CPU pressure â†’ probe misconfig, app bug, or startup dependency failure. Pull events for
  `Unhealthy` and `Killing`.

### Termination reasons

- `OOMKilled` â†’ memory path.
- `Error` â†’ non-zero exit. Check app logs; if empty/minimal, check CPU throttling before attributing to app logic.
- `Completed` â†’ ran to completion. Normal for Jobs/CronJobs/init containers; anomalous otherwise.
- `ContainerCannotRun` â†’ runtime/image/exec issue. Check image pull events.

## Investigation flow

> An investigation is not a checklist. The sections below describe a _typical_ arc â€” **compress, skip, or revisit them
...
**Application logs** if available â€” look at the 200 most recent lines before the termination timestamp. If absent, flag
`no_logs_available`; do not invent a log pattern.

**APM** if the pod runs an instrumented service â€” resolve `service.name` from pod resource attributes for later
correlation. SLO / latency / error-rate analysis itself is APM-layer work and out of scope for this skill.

**Baseline comparison** â€” for utilization-based findings, compare current values to 7-day-prior at the same hour-of-day.
"High memory" is meaningful only relative to what's normal for this workload.

### Check for upstream cause (conditional)

Only pursue if the symptom pattern suggests it. Threshold: upstream error rate >5Ã— baseline _or_ latency >3Ã— baseline,
AND degradation started before the symptom on the target service. Co-symptoms do not establish causation.

If `metrics-service_destination.1m.otel-default` has no rows for the service, report `insufficient_dependency_data` â€”
...
but still a correlation â€” verify it plausibly explains the mode you've classified.

### Synthesize and stop

Synthesize as soon as you have enough evidence to support a hypothesis at known confidence. You do not need to complete
every section above â€” investigation terminates when either:
...
1. <Most actionable â€” typically a config check or metric to observe.>
2. <Secondary.>
...
- If `last_terminated_reason == "OOMKilled"` and memory utilization hit 1.0 â†’ memory path. Corroborate with 7-day
  baseline: monotonic rise over days = leak; spiky = load-driven. Check GC metrics if language is known.
- If `last_terminated_reason == "Error"` and `cpu_limit_utilization > 1.0` â†’ CPU throttling path. Corroborate with
  liveness probe config (initialDelaySeconds, timeoutSeconds) and K8s events for `Unhealthy`.
- If `last_terminated_reason == "Error"` and CPU is fine â†’ application-logic path. Pull recent logs before termination.
- If `last_terminated_reason == "ContainerCannotRun"` â†’ image/exec path. Check K8s events for `Failed` pull events.
...
- K8s events on the new ReplicaSet: `FailedCreate` â†’ admission rejection (quota, webhook, PSP). `FailedScheduling` â†’ no
  node fits.
- New-pod utilization: all at 0% memory â†’ never started (image pull failure); high CPU with low memory â†’ slow startup
  hitting readiness probe.
- HPA state: stable `current_replicas < desired_replicas` under load â†’ unready-pod dampening.
...
- **Workflow:** `K8s CrashLoopBackOff Investigation` â€” alert-triggered automated version of the pod-level path above.
