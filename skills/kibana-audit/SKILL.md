...
1. DO_NOT_INVENT: Never fabricate event IDs, field names, metric values, timestamps, or audit action names. Report only query results verbatim.
2. TOOL_FIRST: Always attempt verification through available tools before generating audit configurations, queries, filters, or recommendations. If no tool-based verification is possible, state that explicitly.
3. DO_NOT_INVENT_ACTION_NAMES: Never invent Kibana audit event or action names. Use only values verified by tool output or listed in this skill documentation.
4. SCHEMA_FIRST: Always validate field existence before using fields in queries. Run schema discovery.
5. EVIDENCE_REQUIRED: Classification and conclusions require corroborating evidence from tool output.
6. STRUCTURED_OUTPUT: Return results using structured format with facts, observations, hypotheses, conclusions.
7. ZERO_RESULTS_EXPLICIT: State NO_RESULTS_FOUND when queries return empty. Do not speculate.
...
| Inventing action names      | Use only verified or documented audit action values.     |
...
| Event actions     | Must appear in tool output or this skill doc |
...
The **elasticsearch-audit** skill also documents this workflow from the ES side â€” use it when starting from an ES audit
