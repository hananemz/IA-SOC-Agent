...
disable, resync) via the Kibana Streams REST API. Streams are an experimental way to manage data in Kibana â€” expect API
...
- **Lifecycle operations:** `POST /api/streams/_disable`, `_enable`, and `_resync` are mutating â€” send `kbn-xsrf: true`
...
- **Stream lifecycle** (`ingest.lifecycle`) â€” Controls how long the **stream's data** is retained. Use
...
- **Failure store lifecycle** (`ingest.failure_store.lifecycle`) â€” Controls retention of **failed documents** only
...
```bash
# Disable streams (request body per API docs) â€” deletes wired stream data; warn and confirm before proceeding
