...
1. DO_NOT_INVENT: Never fabricate IDs, names, endpoints, APIs, settings, privilege names, realm options, security configuration, deployment capabilities, or API responses. Unknown values must be verified or clearly marked as assumptions.
2. TOOL_FIRST: Always attempt verification with available MCP tools, Elasticsearch APIs, Kibana APIs, mappings, or cluster metadata before concluding or generating fixes. If no verification is possible, state that explicitly.
...
4. VERIFY_USER_VALUES: Treat user-provided privilege names, realm names, role names, field names, API names, index names, data streams, and IDs as untrusted until verified. If verification fails, state that the value remains unverified.
5. STOP_ON_UNVERIFIED_SCHEMA: Do not generate DLS queries, ES|QL, Query DSL, mappings, ingest pipelines, or enrich processors from unverified fields. If schema verification failed technically, say so.
...
| Inventing security config | Use only verified or documented settings, realms, and privileges. |
...
| Unverified user values | Verify user-provided names, APIs, IDs, realms, and privileges first. |
| Unverified schema   | Do not generate queries, mappings, or DLS from unverified fields. |
...
| **Authentication**     | Any valid credentials â€” even minimal â€” to reach the cluster                |
...
- `WWW-Authenticate` header â€” indicates which auth schemes the cluster accepts.
- `authentication_exception` in the response body â€” the `reason` field describes what failed.
...
| Paid realm after expiry | License expired â€” see [License Expiry Recovery](#license-expiry-recovery). |
...
| API key returns 403 on previously allowed | API key privileges are a snapshot â€” role changes after |
...
- `SSL certificate problem: unable to get local issuer certificate` â€” CA not trusted.
- `SSL certificate problem: certificate has expired` â€” certificate past its validity date.
- `SSL: no alternative certificate subject name matches target host name` â€” hostname mismatch.
...
On ECH, TLS is managed by Elastic â€” certificate errors usually indicate the client is not using the correct Cloud
...
| Derived key has no access                 | API key created another API key â€” derived keys have no privilege |
...
| Serverless   | Not applicable â€” licensing is fully managed by Elastic.                   |
...
Response: `"has_all_requested": false` â€” the `viewer` role does not include `read` on `logs-*`.
...
Response shows `"expiration": 1709251200000` â€” the key expired.
...
1. Check for clock skew â€” SAML assertions are time-sensitive. Ensure NTP is configured on all nodes.
...
- TLS is managed by Elastic â€” certificate errors typically indicate an incorrect endpoint URL.
...
- Native users do not exist â€” authentication issues are handled at the organization level.
