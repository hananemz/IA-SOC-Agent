...
1. DO_NOT_INVENT: Never fabricate event IDs, field names, metric values, timestamps, settings, privilege names, APIs, mappings, role names, realm options, or security configuration. Unknown values must be verified or clearly marked as assumptions.
...
4. VERIFY_USER_VALUES: Treat user-provided privilege names, realm names, role names, field names, mappings, API names, index names, data streams, and IDs as untrusted until verified. If verification fails, state that the value remains unverified.
5. SCHEMA_FIRST: Always validate field existence before using fields in queries. Run schema discovery.
6. STOP_ON_UNVERIFIED_SCHEMA: Do not generate DLS queries, ES|QL, Query DSL, mappings, ingest pipelines, or enrich processors from unverified fields. If schema verification failed technically, say so.
...
| Unverified user values      | Verify user-provided names, APIs, IDs, and privileges first. |
...
### Step 2 â€” Check for existing roles
...
### Step 3 â€” Create the role (if needed)
...
### Step 4 â€” Create or update the user
...
1. Check roles: `GET /_security/role` â€” no match.
...
> organization level â€” skip this section.
...
privileges. This is the default â€” no Kibana endpoint is required.
...
same role works for all departments â€” no per-department role needed.

For multi-valued attributes (e.g. a list of required programs), use `terms_set` with `minimum_should_match_field` to
ensure the user holds all required attributes listed on the document. This enables complex ABAC policies â€” combining
security levels, program lists, and certification dates â€” using a single role. See
...
> Self-managed and ECH only. On Serverless, use the **cloud-access-management** skill â€” see
...
The `roles` array is **replaced entirely** â€” include all roles the user should have. Fetch the user first to see current
...
> **cloud-access-management** skill instead â€” see [Serverless User Access](#serverless-user-access).
...
- **Predefined roles** (e.g. `admin`, `developer`, `viewer`) cover common access patterns. If one fits, assign it
  directly via the Cloud API â€” no custom role creation needed.
- **Custom roles** are required when the user needs fine-grained access (specific indices, Kibana features, DLS/FLS).
  Create the custom role using the Elasticsearch API or Kibana API (same as self-managed â€” see
...
1. `GET /_security/user/alice` â€” response shows `"roles": ["viewer"]`.
...
The same role works for every department â€” each user sees only their department's records with PII fields removed.
...
- Set `description` to a short, human-readable display name â€” not a long sentence. It is shown in the Kibana UI as the
