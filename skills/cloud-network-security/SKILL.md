...
API uses **traffic filters** â€” you will see `traffic-filters` in endpoint paths and `traffic_filters` in JSON fields.
...
> filtering), create the VPC endpoint and DNS record in AWS â€” no filter is needed on the Elastic Cloud side.
...
- **Filter hygiene â€” reuse, scope, and clean up:**
  ...
  - Duplicate filters means filters for the same purpose with identical source rules â€” not merely overlapping IPs. Two
...
- `include_by_default` automatically associates the filter with all new projects in the region. Use with caution â€” it
