...
All connector types share the same request structure â€” only `connector_type_id`, `config`, and `secrets` differ. See the
...
`PUT /api/actions/connector/{id}` replaces the full configuration. `connector_type_id` is immutable â€” delete and
...
`referenced_by_count` â€” if non-zero, reassign the referencing rules first, then `DELETE /api/actions/connector/{id}`.

## Guidelines

- Include `kbn-xsrf: true` on every POST, PUT, and DELETE; omitting it returns 400.
- `connector_type_id` is immutable â€” delete and recreate to change connector type.
