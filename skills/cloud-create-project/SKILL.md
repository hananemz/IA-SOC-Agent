...
> [Elastic Cloud free trial](https://cloud.elastic.co/registration) â€” 14 days of full access with no credit card
...
- **Admin credentials are for API key creation only.** The script saves the `admin` password to `.elastic-credentials`
  ...
  use admin credentials for direct Elasticsearch operations (querying, indexing, etc.) â€” always create a scoped API key
  first (see Step 8). The `load-credentials` command excludes admin credentials by default â€” use `--include-admin`
...
Elasticsearch projects do not have a product tier â€” use `--optimized-for` instead.
...
Project type must be confirmed with the user â€” do not assume a default. See "Project type inference" above.

Always use `--optimized-for general_purpose` unless the user explicitly requests `vector`. Do not proactively offer the
`vector` option.

If the user does not specify a name, ask for one â€” it is required.
...
```text
Credentials saved to .elastic-credentials â€” open that file to retrieve your password.
```
...
- **Project ID** â€” needed for all subsequent operations
- **Cloud ID** â€” for client libraries
- **Elasticsearch and Kibana endpoints** â€” safe to display in chat
...
If the `elasticsearch-authn` skill is available, use it for API key creation â€” it covers the full lifecycle (create,
...
- Region cannot be changed after creation â€” confirm the choice before proceeding.
