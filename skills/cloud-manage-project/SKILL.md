...
- **Never display secrets in chat.** Do not echo, log, or repeat API keys, passwords, or credentials in conversation
  ...
  **never** appear in chat history, thinking traces, or agent output â€” even when using it to create an API key, pass it
...
- **Admin credentials are for API key creation only.** The `admin` password saved by `create-project` and
  `reset-credentials` exists solely to bootstrap a scoped API key â€” never use it for direct Elasticsearch operations.
...
self-managed or Elastic Cloud Hosted, this skill does not apply â€” skip it and proceed with the relevant skill directly.
...
This sets all saved environment variables for the project â€” endpoints and any previously created Elasticsearch API keys
â€” in a single command. Admin credentials (`ELASTICSEARCH_USERNAME`/`ELASTICSEARCH_PASSWORD`) are intentionally excluded.
...
**If yes** â€” have the user add it to `.elastic-credentials` (see "Credential file format"). Do not accept keys in chat.
Reload and verify:
...
**If no** â€” follow this recovery path:
...
credentials (`ELASTICSEARCH_USERNAME`/`ELASTICSEARCH_PASSWORD`) are excluded by default â€” only endpoints and API keys
...
Tags are sent as `metadata.tags` in the API request. Setting tags replaces all existing tags on the project â€” include
...
queried data â€” this cached data is considered **search-ready**.

| Flag             | Range   | Description                                                                  |
| ---------------- | ------- | ---------------------------------------------------------------------------- |
| `--search-power` | 28â€“3000 | Query performance level. Higher values improve performance but increase cost |
| `--boost-window` | 1â€“180   | Days of data eligible for boosted caching (default: 7)                       |
...
When the user asks for a preset by name, map it to the corresponding value. Custom values within 28â€“3000 are also valid.
...
- **Maximum retention** â€” enforces an upper bound across all data streams. When lowered, it replaces the retention for
  any stream that currently has a longer period. Data older than the new maximum is **permanently deleted**.
- **Default retention** â€” automatically applied to data streams that do not have a custom retention period set. Does not
...
The new password is saved to `.elastic-credentials` automatically. Tell the user to open that file â€” do not display its
contents in chat.
...
Agent: Let me find that project.
       ...
       Verified â€” authenticated as user "my-service" via API key. You're all set.
```
...
Agent: [runs: reset-credentials --type observability --id def456...]
       Credentials saved to .elastic-credentials â€” open that file to retrieve the password.
       ...
       now â€” what access does it need?
...
- Use the `cloud-create-project` skill to create new projects â€” this skill handles existing projects only.
...
| `--search-power`           | update                                                           | Search power 28â€“3000 (elasticsearch only)                    |
| `--boost-window`           | update                                                           | Boost window 1â€“180 days (elasticsearch only)                 |
