...
given deployment typically uses **one or more** ingestion paths (APM/OTLP traces **and/or** integration metrics/logs)â€”
...
where supported **logs**) to Elastic. Metrics typically include token usage, request counts, latency, andâ€”where the
integration supports itâ€”cost-related fields. Logs may include prompt/response or guardrail events. Exact field names and
...
- **Traces only:** Use **trace hierarchy** in `traces*`. Filter by root service or trace attributes; group by `trace.id`
  and use parent/child span relationships (e.g. `parent.id`, `span.id`) to reconstruct chains (e.g. orchestration span â†’
...
Vertex AI, etc.), use that integrationâ€™s data stream index pattern and field names from its package (see
[Elastic LLM observability](https://www.elastic.co/docs/solutions/observability/applications/llm-observability)).

## Guidelines

- **Data only in Elastic:** Use only data collected and stored in Elastic (traces in `traces*`, metrics, or integration
  metrics/logs). Do not describe or rely on other vendorsâ€™ UIs or products.
...
- **References:**
  ...
  [Observability Labs â€“ LLM Observability](https://www.elastic.co/observability-labs/blog/tag/llmobs),
