...
- **Example patterns:** Throughput, latency, and error rate over time: see Kibana `trace_charts_definition.ts`
  (`getThroughputChart`, `getLatencyChart`, `getErrorRateChart`). Use `from(index)` â†’ `where(...)` â†’ `stats(...)` /
...
- From the serviceâ€™s traces or metrics, read resource attributes such as `k8s.pod.name`, `k8s.namespace.name`,
